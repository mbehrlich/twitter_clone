class User < ActiveRecord::Base
  ...
  def feed_tweets(limit = nil, max_created_at = nil)
    max_created_at ||= DateTime.now
    @tweets = Tweet
      .joins(:user)
      .joins("LEFT OUTER JOIN follows ON users.id = follows.followee_id")
      .where("(tweets.user_id = :id OR follows.follower_id = :id) AND tweets.created_at < :max", id: self.id, max: max_created_at)
      .order("tweets.created_at DESC")
      .limit(limit)
      .uniq

      @tweets.each do |tweet|
        next if max_created_at.nil?
        p max_created_at > tweet.created_at
      end
